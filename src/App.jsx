...
function App() {
  ...
  const handleAddOption = async (labelType) => {
    const val = customOptionValue.trim();
    if (!val) return;
    try {
      const parent =
        labelType === "attribute" && selectedAspect ? selectedAspect : null;
      await addLabelOption({ labelType, value: val, parentValue: parent });
      setConfig(await getLabelOptions());
      setAddingOption(null);
      setCustomOptionValue("");
      if (labelType === "aspect") setSelectedAspect(val);
      else if (labelType === "attribute") setSelectedAttribute(val);
      else if (labelType === "sentiment") setSelectedSentiment(val);
    } catch (err) {
      alert("Add option error: " + err.message);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 to-violet-500">
      ...
      {pageLoading ? (
        <div className="mx-auto max-w-7xl px-4 flex items-center justify-center" style={{ minHeight: "50vh" }}>
          <p className="text-white text-xl font-semibold animate-pulse">
            Loading…
          </p>
        </div>
          ...
          <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-4">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    setAddingOption("aspect");
                    setCustomOptionValue("");
                  }}
                  className="text-xs text-indigo-500 font-semibold hover:text-indigo-700 transition cursor-pointer"
                >
                  + Add
                </button>
              </div>
              {addingOption === "aspect" && (
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") handleAddOption("aspect");
                      if (e.key === "Escape") setAddingOption(null);
                    }}
                  />
                  <button
                    onClick={() => handleAddOption("aspect")}
                    className="rounded bg-indigo-500 px-3 py-1 text-xs font-semibold text-white hover:bg-indigo-600 transition cursor-pointer"
                  >
                    Add
                  </button>
                </div>
              ...
              {addingOption === "attribute" && (
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    onChange={(e) => setCustomOptionValue(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") handleAddOption("attribute");
                      if (e.key === "Escape") setAddingOption(null);
                    }}
                  />
                  <button
              ...
              {addingOption === "sentiment" && (
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    onChange={(e) => setCustomOptionValue(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") handleAddOption("sentiment");
                      if (e.key === "Escape") setAddingOption(null);
                    }}
                  />
                  <button
                    onClick={() => handleAddOption("sentiment")}
                    className="rounded bg-indigo-500 px-3 py-1 text-xs font-semibold text-white hover:bg-indigo-600 transition cursor-pointer"
                  >
                    Add
                  </button>
